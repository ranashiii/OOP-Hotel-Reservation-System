...
public class GuestProfilePanel extends JPanel {
    ...
    /**
     * Loads the guest profile from the database into the form fields.
     * If no guest record exists yet (new user), the form is left blank
     * so the user can fill it in and save for the first time.
     */
    private void loadGuestProfile() {
        try {
            ...
            // No guest row yet — leave the form blank, not an error
    ...
    /**
     * Saves the profile. If no guest record exists yet (currentGuest == null),
     * creates a new one via registerGuest(). Otherwise updates the existing record.
     */
    private void saveProfile() {
        try {
            ...
            // Parse date of birth — applies to both create and update paths
            java.time.LocalDate dob = java.time.LocalDate.parse(txtDateOfBirth.getText());

            if (currentGuest == null) {
                // First-time save: no guest row exists yet — create one
                Guest newGuest = new Guest();
                newGuest.setUserId(SessionManager.getInstance().getCurrentUserId());
                newGuest.setFirstName(txtFirstName.getText());
                newGuest.setMiddleName(txtMiddleName.getText());
                newGuest.setLastName(txtLastName.getText());
                newGuest.setEmail(txtEmail.getText());
                newGuest.setPhoneNumber(txtPhone.getText());
                newGuest.setAddress(txtAddress.getText());
                newGuest.setDateOfBirth(java.sql.Date.valueOf(dob));
                newGuest.setNationality(txtNationality.getText());
                newGuest.setIdDocumentType(txtIdType.getText());
                newGuest.setIdDocumentNumber(txtIdNumber.getText());
                // registerGuest() also validates age (18+) and other fields
                currentGuest = guestService.registerGuest(newGuest);
            } else {
                // Existing guest — update in place
            ...
            MessageBox.showError("Error", "Failed to save profile: " + e.getMessage());
        }
    }
